...
SUPPORTED_CAPTURE_MODES = {"single", "multi", "single_and_multi"}
...
def get_dataset_capture_mode(supervisor: Supervisor) -> str:
    world_mode = supervisor.getCustomData().strip()
    if world_mode.startswith("capture_mode="):
        world_mode = world_mode.split("=", 1)[1].strip()

    if world_mode in SUPPORTED_CAPTURE_MODES:
        return world_mode

    return DATASET_CAPTURE_MODE
...
def get_capture_root_name(capture_mode: str) -> str:
    if capture_mode == "single":
        return "captures_single"
    if capture_mode == "multi":
        return "captures_multi"
    return "captures"
...
def build_capture_plan(capture_mode: str):
    object_pool = get_capture_object_pool()

    if capture_mode == "single":
        return build_single_object_scenes(object_pool)
    ...
    if capture_mode == "single_and_multi":
...
def run_capture_sequence(supervisor: Supervisor, timestep: int, object_list, capture_mode: str):
    ...
    capture_root = get_capture_root_name(capture_mode)
...
def run_scene(
    ...
    return run_capture_sequence(supervisor, timestep, object_list, capture_mode)


def main(capture_mode_override=None):
    supervisor = Supervisor()
    timestep = int(supervisor.getBasicTimeStep())
    capture_mode = capture_mode_override or get_dataset_capture_mode(supervisor)
    capture_plan = build_capture_plan(capture_mode)

    print(f"[Supervisor] Dataset mode: {capture_mode}")
