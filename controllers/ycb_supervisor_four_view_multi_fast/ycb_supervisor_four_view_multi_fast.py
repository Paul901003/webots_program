...
CAPTURES_DIR        = os.path.join(DATA_DIR, "captures_fast", "multi")
UR5E_DEF            = "UR5E"
...
def wait_for_capture(supervisor, timestep, pose_json_path, timeout_sec):
    """輪詢 realsense 寫出 _pose.json(save_capture 最後寫的檔=完成標記)取代盲等 CAPTURE_WAIT。
    呼叫前需先刪舊 _pose.json,確保出現的是本次觸發的新檔;timeout_sec 為上限。
    回傳 False 僅代表模擬結束(-1);逾時仍回 True(best-effort,與原盲等行為一致但有警告)。"""
    deadline = supervisor.getTime() + timeout_sec
    while supervisor.step(timestep) != -1:
        try:
            if os.path.getsize(pose_json_path) > 0:
                return True
        except OSError:
            pass
        if supervisor.getTime() >= deadline:
            print(f"[Supervisor] ★拍攝寫檔逾時 {timeout_sec}s: {os.path.basename(pose_json_path)}")
            return True
    return False
...
def run_scene(supervisor, timestep, emitter, receiver, camera_node, scene, path_dict=None, visit_order=None, captures_dir=None):
    ...
    if captures_dir:
        ...
        base_dir = os.path.join(_ARMMOVE_ROOT or os.path.join(DATA_DIR, "captures_fast"),
                                f"multi_{group}")
    ...
    for vp in viewpoints:
        ...
        # 先刪舊 _pose.json(realsense 存檔最後寫它)→ 之後輪詢它出現=本次寫檔完成
        pose_json_path = os.path.join(scene_dir, f"{view_name}_pose.json")
        try:
        ...
        # 輪詢寫檔完成取代盲等(CAPTURE_WAIT_SEC 當上限逾時)→ 通常 <0.5s 即返回
        if not wait_for_capture(supervisor, timestep, pose_json_path, CAPTURE_WAIT_SEC):
            return False
...
def main():
    ...
    if capture_scene:
        scenes = None
        for pf in [SCENE_PLAN_PATH, SINGLE_SCENE_PLAN_PATH] + EXTRA_SCENE_PLANS:
