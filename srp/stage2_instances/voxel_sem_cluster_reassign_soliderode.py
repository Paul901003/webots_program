...
"""voxel_sem_cluster_reassign_soliderode.py — 「侵蝕判小塊」變體(複製自 voxel_sem_cluster_reassign_soliddrop.py)。

★ 與 soliddrop 唯一差別:小塊(<MIN_VOX)的「併/丟」決策改用**實心 occupancy 侵蝕 1 層判定**(只判定、不刪任何 voxel):
    - 大塊(≥MIN_VOX):照原本正常連通各自成 instance,**完全不侵蝕**(細長物不會被過切)。
    - 小塊:侵蝕後若和某相鄰大實例仍落在「同一撐過侵蝕的粗塊(chunk)」= 連接夠粗、真連 → 併進該大實例;
      侵蝕後和大實例斷開(連接只有單/細 voxel、被侵蝕切斷=鬼影虛連)或小塊本身太細整塊被蝕掉 → 丟(留 label 0);
      本來就不碰任何大塊 → 丟(同 soliddrop 現況)。
    - 侵蝕全程只當判定;併進去的小塊 voxel 一個不刪、體積全保留。
  取代 soliddrop 的「相鄰(dilation 觸碰)就併」——那會把靠單一 ghost voxel 虛連的小碎片誤黏到大物體。
  其餘(表面 voxel、實心遮擋 zbuffer、footprint/center 投票、分群、去大遮罩、div 下游)全同,不改原檔。

--- 原 docstring(soliddrop / surf_donut)---
...
可視化: SRP_VIZ_ARGS="<scene> 1 <out_root> cubes" webots worlds/hull_viz.wbt
用法: ./voxel_sem_cluster_reassign_soliderode.py [scene|group|(空=全部)] [--n-views 12] [--sem-thr 0.4]
env: SAM_ROOT HULL_ROOT CAPTURES_ROOT ARM_MASK_ROOT OUT_ROOT NEST_THR(0.8) VOTE(footprint|center) MIN_VOX(50)
"""
...
OUT_ROOT = REPO / "data" / "eval" / os.environ.get("OUT_ROOT", "srp_hull_semcluster_soliderode")
...
def semantic_cluster(sc, n_views, sem_thr):
    ...
    if gl:
        ...
        nid = 0
        for comp_p, gid in big:                                    # 大塊各自成實例(不侵蝕,細長物不被過切)
            nid += 1; labels[tuple(vox[comp_p].T)] = nid; inst_masks[nid] = imask(comp_p, gid)
        # ★ 小塊(<MIN_VOX):用「實心 occupancy 侵蝕 1 層」判定和相鄰大實例的連接是真連還是單/細 voxel 虛連。
        #   侵蝕只當判定、不刪任何 voxel;併進去的小塊 voxel 一個不刪、體積全保留。
        big_snapshot = labels.copy()                               # 相鄰/同 chunk 對象只看「大實例」,不受小塊處理順序影響
        if small:
            st_er = ndimage.generate_binary_structure(3, 1)        # 6 連通侵蝕(單/細 voxel 連接才會被切斷)
            er = ndimage.binary_erosion(SOLID.astype(bool), st_er) # ★侵蝕 1 層(判定用,不改 occupancy)
            lab_e, _ne = ndimage.label(er, st)                     # 撐過侵蝕的粗塊(chunk)
            inv = lab_e == 0
            dist, idx = ndimage.distance_transform_edt(inv, return_indices=True)
            chunk = np.where((dist <= 1.5) & SOLID.astype(bool), lab_e[tuple(idx)], 0)  # 回填被侵蝕掉的表面殼(1 層)
            cg_at = chunk[tuple(vox.T)]                             # 每 surface voxel 的 chunk id(0=侵蝕後無粗塊)
            bl = big_snapshot[tuple(vox.T)]
            big_chunks = {}                                        # 大實例 → 它佔到的 chunk 集合(撐過侵蝕的粗塊)
            for gid_big in np.unique(bl[bl > 0]):
                big_chunks[int(gid_big)] = set(cg_at[(bl == gid_big) & (cg_at > 0)].tolist())
            for comp_p, gid in small:
                ...
                if neigh.size == 0:
                    continue                                       # 本來就不碰大塊 → 丟(同 soliddrop)
                my_chunks = set(cg_at[comp_p][cg_at[comp_p] > 0].tolist())   # 小塊撐過侵蝕的 chunk
                u, c = np.unique(neigh, return_counts=True)
                cand = [(int(gg), int(cc)) for gg, cc in zip(u, c)          # 相鄰且「同 chunk(真連)」的大實例
                        if my_chunks & big_chunks.get(int(gg), set())]
                if cand:
                    cand.sort(key=lambda x: -x[1])
                    labels[tuple(vox[comp_p].T)] = cand[0][0]      # 併進「同 chunk 且相鄰最多」的大實例
                # else:連接被侵蝕切斷(單/細 voxel 虛連)或小塊太細整塊被蝕掉 → 丟(留 label 0)
...
def process(sc, n_views, sem_thr):
    ...
    meta = {"script": "voxel_sem_cluster_reassign_soliderode.py", "vote": VOTE, "occluder": "solid",
            "reassign": "connected_by_erosion", "erode_conn": 6, "surface": True, "zbuffer": True, "donut": True,
    ...
    print(f"[{sc}] 侵蝕判小塊 → {len(insts)} instance", flush=True)
