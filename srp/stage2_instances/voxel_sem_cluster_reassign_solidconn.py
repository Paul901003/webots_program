    ...
    if gl:
        ...
        for comp_p, gid in small:
            ...
            if neigh.size > 0:
                ...
                nid += 1; labels[tuple(vox[comp_p].T)] = nid; inst_masks[nid] = imask(comp_p, gid)  # 不相連→自成一群
...
def process(sc, n_views, sem_thr):
    ...
    meta = {"script": "voxel_sem_cluster_reassign_solidconn.py", "vote": VOTE, "occluder": "solid", "reassign": "connected_only", "surface": True, "zbuffer": True, "donut": True,
