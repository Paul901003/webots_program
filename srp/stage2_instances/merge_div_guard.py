...
def guarded_union_find(nodes, edges, complete, order_key=None):
    """完整度守衛版 union-find:每個 component 記「是否已含原始完整群」;
    若一條邊要合併的兩個 component 都已含完整群 → 拒絕(不讓兩個完整物經碎片橋接串起)。
    碎片(不含完整群)仍可黏到某完整物。edges 依 order_key 由大到小處理(強 span 先),
    保證『兩完整物不同群』與順序無關,順序只影響碎片黏哪邊。"""
    par = {n: n for n in nodes}
    has = {n: bool(complete.get(n, False)) for n in nodes}   # 該 root 的 component 是否含完整群
    def find(x):
        while par[x] != x:
            par[x] = par[par[x]]; x = par[x]
        return x
    es = sorted(edges, key=order_key, reverse=True) if order_key else list(edges)
    for a, b in es:
        ...
        if ra == rb:
            continue
        if has[ra] and has[rb]:          # ★兩塊都已含完整群 → 拒絕橋接
            continue
        par[ra] = rb
        has[rb] = has[ra] or has[rb]
...
def main():
    ...
    for i, sc in enumerate(scenes):
        try:
            ...
            for th in THETAS:
                tag = f"t{int(th*100)}"
                complete = {g: (div.get(g, 0) >= th) for g in gids}          # 原始群是否完整
                ordk = lambda e: sf.get((min(e[0], e[1]), max(e[0], e[1])), 0.0)  # 強 span 先併
                # A:3D 連通對,守衛式(兩塊都含完整群→不併)
                gA = guarded_union_find(gids, c3, complete, ordk)
                write_root(sc, bg, gA, f"srp_hull_divA_{tag}{SUFFIX}",
                           {"script": "merge_div_guard.py", "method": "A_gate_guard", "theta": th, "src": M.BASE})
                # B:span 邊,守衛式(取代原 both_complete 逐邊篩;守衛擋直接+傳遞的兩完整物合併)
                gB = guarded_union_find(gids, span_edges, complete, ordk)
                write_root(sc, bg, gB, f"srp_hull_divB_{tag}{SUFFIX}",
                           {"script": "merge_div_guard.py", "method": "B_span_guard", "theta": th, "span_thr": SPAN_THR, "src": M.BASE})
