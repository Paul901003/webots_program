...
def semantic_cluster(sc, n_views, sem_thr):
    ...
    # ★ 連通改用 hull occupancy(佔據)連通塊:同語意群但落在不同 occupancy 塊(無法經佔據 voxel 互通)→ 拆成不同實例
    _csrc = z["surface"] if os.environ.get("CONN_SRC", "occ") == "surf" else z["occupancy"]  # 連通媒介:occ(佔據含內部)/surf(表面)
    occ_lab, _ = ndimage.label(_csrc, ndimage.generate_binary_structure(3, 3))
    vox_occ = occ_lab[tuple(vox.T)]                          # 每候選 voxel 所屬的連通塊 id
    ...
    if gl:
        ...
        for gi in range(len(gl)):
            ...
            for cc in np.unique(vox_occ[sel_p]):            # ★依 occupancy 連通塊拆(取代原表面 26-鄰接)
                if cc == 0: continue
                comp_p = sel_p[vox_occ[sel_p] == cc]
                if len(comp_p) < MIN_VOX: continue
                nid += 1; labels[tuple(vox[comp_p].T)] = nid
                gid = gl[gi]
                md = defaultdict(set)
                for p in comp_p:
...
def process(sc, n_views, sem_thr):
    ...
    meta = {"script": "voxel_sem_cluster_connected.py", "connectivity": "hull_occupancy_26",
