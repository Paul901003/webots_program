...
echo "######## voxel大小完整流程 5尺寸×60場 START $(date '+%m-%d %H:%M') ########"
for MM in 10 7 5 3 1; do
  V=$(python3 -c "print($MM/1000)"); HR=srp_hull_vox${MM}mm
  CK=4000000; [ $MM -le 3 ] && CK=2000000; [ $MM -eq 1 ] && CK=1500000
  echo "==== ${MM}mm 建hull $(date '+%H:%M') ===="
  for sc in $SCENES; do
    [ -f data/eval/$HR/$sc/hull.npz ] && continue
  ...
  echo "   hull:$(ls data/eval/$HR/*_scene*/hull.npz 2>/dev/null|wc -l) surface+分群 $(date '+%H:%M')"
  $PY srp/stage2_instances/add_surface_mask.py stack3 stack4 stack5 --root $HR >/dev/null 2>&1
  FORCE=1 HULL_ROOT=$PWD/data/eval/$HR SAM_ROOT=$MV2 CAPTURES_ROOT=$CAP OUT_ROOT=srp_semdonut_vox${MM}mm \
    $PY srp/stage2_instances/voxel_sem_cluster_surf_donut.py stack3 stack4 stack5 --n-views 12 --sem-thr 0.4 >/dev/null 2>&1
  echo "   分群:$(ls data/eval/srp_semdonut_vox${MM}mm/*_scene*/instances.json 2>/dev/null|wc -l)場 $(date '+%H:%M')"
done
echo "######## 建置DONE $(date '+%H:%M') ########"
