...
"""voxel_sem_cluster_connected.py — surf_donut 變體:連通判定改用 hull occupancy(佔據)連通塊。
= voxel_sem_cluster_surf_donut.py,唯一差異:同一語意群的表面 voxel,若落在不同 occupancy 連通塊
  (無法經佔據 voxel 互通)則拆成不同實例(取代原本「表面 voxel 26-鄰接」)。相觸/堆疊 hull 融成
  一塊→不拆;分離不相觸→拆;單一物體表面破碎→不誤拆。其餘(donut/drop_arm/sem_thr/min_vox)全同。
--- 以下為原 surf_donut 說明 ---
surf(表面 voxel+z-buffer) × donut(去大遮罩) 組合版。
...
OUT_ROOT = REPO / "data" / "eval" / os.environ.get("OUT_ROOT", "srp_hull_semcluster_connected_am1photo")
...
def semantic_cluster(sc, n_views, sem_thr):
    ...
    # ★ 表面跨無標籤 region-grow + 完整度守衛(取代 occupancy 分塊)
    gl = list(votes.keys()); inst_masks = {}
    if gl:
        Vt = np.stack([votes[g] for g in gl], 1); assign = Vt.argmax(1); has = Vt.max(1) > 0
        grp = np.zeros(M, np.int32); ga = np.array(gl)          # 每候選 voxel 的群(0=無標籤)
        grp[has] = ga[assign[has]]
        # 每群完整度 div=1-‖mean unit normal‖(occupancy 梯度,群==g 上算);div≥THC=完整
        THC = float(os.environ.get("DIV_THETA", "0.5"))
        grp3 = np.zeros(shape, np.int32); grp3[tuple(vox.T)] = grp
        g0, g1, g2 = np.gradient(z["occupancy"].astype(float))
        complete = {}
        for g in gl:
            mm = (grp3 == g); grad = np.stack([g0[mm], g1[mm], g2[mm]], 1)
            mag = np.linalg.norm(grad, axis=1); keep = mag > 1e-6
            complete[g] = bool(keep.sum() >= 20 and float(1 - np.linalg.norm((grad[keep] / mag[keep, None]).mean(0))) >= THC)
        # 表面 26-鄰接 union-find:同群→連、有無標籤→橋、不同已標群→不連;守衛=兩「含完整群」塊不橋
        coord2idx = {(int(c[0]), int(c[1]), int(c[2])): i for i, c in enumerate(vox)}
        offs3 = [(dx, dy, dz) for dx in (-1, 0, 1) for dy in (-1, 0, 1) for dz in (-1, 0, 1) if (dx or dy or dz)]
        par = list(range(M)); hasc = [bool(grp[p] > 0 and complete.get(int(grp[p]), False)) for p in range(M)]
        def find(x):
            r = x
        ...
        same = []; bridge = []
        for p in range(M):
            cx, cy, cz = int(vox[p][0]), int(vox[p][1]), int(vox[p][2])
            for dx, dy, dz in offs3:
                q = coord2idx.get((cx + dx, cy + dy, cz + dz))
                if q is None or q <= p: continue
                gp, gq = int(grp[p]), int(grp[q])
                if gp > 0 and gq > 0:
                    if gp == gq: same.append((p, q))            # 同群→連;不同群→不連
                else:
                    bridge.append((p, q))                       # 至少一無標籤→橋
        for a, b in same:
            ra, rb = find(a), find(b)
            if ra != rb: par[ra] = rb; hasc[rb] = hasc[ra] or hasc[rb]
        for a, b in bridge:
            ra, rb = find(a), find(b)
            if ra == rb: continue
            if hasc[ra] and hasc[rb]: continue                  # ★守衛:兩完整物不橋
            par[ra] = rb; hasc[rb] = hasc[ra] or hasc[rb]
        comp = defaultdict(list)
        for p in range(M): comp[find(p)].append(p)
        nid = 0
        for root, ps in comp.items():
            ps = np.array(ps)
            if len(ps) < MIN_VOX: continue
            gg = grp[ps]; gg = gg[gg > 0]
            if len(gg) == 0: continue                           # 純無標籤元件不出實例
            nid += 1; labels[tuple(vox[ps].T)] = nid
            gid = int(np.bincount(gg).argmax())
            md = defaultdict(set)
            for p in ps:
                for vw, fs in vmask.get(int(p), {}).items():
                    for fn in fs:
                        if mask_cluster.get(vw, {}).get(fn) == gid: md[vw].add(fn)
...
def process(sc, n_views, sem_thr):
    ...
    meta = {"script": "voxel_sem_cluster_bridge.py", "connectivity": "surface_bridge_unlabeled+guard",
            "surface": True, "zbuffer": True, "donut": True,
