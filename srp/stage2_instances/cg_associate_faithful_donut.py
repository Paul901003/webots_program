...
import cg_associate as CGA           # noqa: E402  重用 detection 端 helper(z-buffer/arm)
from voxel_sem_cluster_donut import donut_masks, donut_feats   # noqa: E402  去大遮罩(像素級挖洞)+重算甜甜圈CLIP
...
NO_DENOISE = os.environ.get("NO_DENOISE", "") == "1"   # opt-in:關掉 DBSCAN 去噪(直接回原 voxel,不丟雜點)


def dbscan_largest(li, P):
    """CG pcd_denoise_dbscan 的 voxel 版:先 downsample 到 DOWNSAMPLE 格,DBSCAN(eps,min)取最大 cluster,
    再回填「落在最大 cluster 格內」的原生 voxel。回傳保留的 local index 陣列。NO_DENOISE=1 時直接回原樣。"""
    li = np.asarray(list(li), np.int64)
    if NO_DENOISE or len(li) < DBSCAN_MIN:
        return li                                   # 關去噪 / 太少 → 回原樣(同 CG:cluster<5 就退回原樣)
    pts = P[li]
...
def process(scene, n_views, out_root):
    ...
    for vn in views:
        ...
        masks_sub = donut_masks(ms0)              # ★去大遮罩:像素級挖洞(取代官方 bbox mask_subtract_contained)
        featmap = donut_feats(vd, rgb, masks_sub, names)   # ★重算甜甜圈 CLIP(取代原始遮罩預存特徵)
        if not featmap:
            continue
        H, W = km[0][0].shape
...
def main():
    ...
    args = ap.parse_args()
    out_root = os.environ.get("OUT_ROOT", "srp_hull_cgf_donut")
    if not args.targets:
