...
def carve_visual_hull(
    ...
    Pidx = torch.stack([gi.reshape(-1), gj.reshape(-1), gk.reshape(-1)], dim=1)
    Pall = gmin + (Pidx + 0.5) * voxel_size      # (M,3) 世界座標 voxel 中心
    M = Pall.shape[0]
    ...
    # ★分批:voxel 之間獨立,把 M 切成 chunk 逐塊算(GPU 記憶體隨 voxel 數暴增時避免 OOM)。
    #   結果(keep_votes/observed)是小 int16/bool,累積在 GPU;大 tensor(X/u/v)只在 chunk 內。
    CHUNK = int(os.environ.get("CARVE_CHUNK", "4000000"))   # 每批 voxel 數
    # 預先把各視角參數搬 GPU(不重複搬)
    view_params = []
    for i in range(V):
        ...
        H, W = mask.shape
        view_params.append((Rt, tt, mask, H, W,
                            float(K[0, 0]), float(K[1, 1]), float(K[0, 2]), float(K[1, 2])))

    for s in range(0, M, CHUNK):
        e = min(s + CHUNK, M)
        P = Pall[s:e]                            # (m,3) 本批 voxel
        m = P.shape[0]
        kv = torch.zeros(m, dtype=torch.int16, device=dev)
        obs = torch.zeros(m, dtype=torch.bool, device=dev)
        for (Rt, tt, mask, H, W, fx, fy, cx, cy) in view_params:
            X = P @ Rt.T + tt
            z = X[:, 2]
            in_front = z > 0.0
