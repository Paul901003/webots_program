...
for i, sc in enumerate(scenes):
    try:
        ...
        variants = {"am0": occ0, "am0+光度雕": occ_photo, "am0+語意重疊雕": occ_mix, "am0+光度+語意雕": occ_pm}
        am1p = Path(f"data/eval/srp_hull_mv2_v12_am1/{sc}/hull.npz")
        if am1p.is_file(): variants["am1(現行)"] = np.load(am1p)["occupancy"]
...
order = ["am1(現行)", "am0", "am0+光度雕", "am0+語意重疊雕", "am0+光度+語意雕"]
for name in order:
